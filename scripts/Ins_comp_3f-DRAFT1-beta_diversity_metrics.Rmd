...
next, use PERMDISP (permutational analysis of multivariate dispersions) to test whether the differences are due to differences in dispersion (variance) or locations of controids - draft stuff with the help of chatgpt - needs checking
...
2.Test for significance of dispersion differences: Use permutest() to assess whether there are statistically significant differences in dispersion among groups.
...
chatgpt suggested I could visualise the outputs as boxplots - delete this for github, not really needed

```{r}
# Boxplot of distances to centroid for each grouping variable
boxplot(disp_sample_type, main = "Dispersion by Sample_Type2")
boxplot(disp_building, main = "Dispersion by Building")
boxplot(disp_interaction, main = "Dispersion by Sample_Type2:Building")

# Plot ordination to visually inspect dispersion
plot(disp_sample_type)
plot(disp_building)
plot(disp_interaction)
...
# betadisperser doesn't support nested designs, so i need to run separately for each building. chat GPT suggested to basically subset the data for each sample type by building, but  don't think this is correct and really I should recalculate for each sample type + building combo separately, then look at differences between trays/cages. Here this is a long way round of doing this and really not efficient, but I will get results then look at a function to repeatedly do this


# A
ps_adult_A <- subset_samples(ps_adult, Building == "A-CTID")
adult_A_bray_dist = phyloseq::distance(ps_adult_A, method="bray", weighted=F)
adult_A_sample_data <- as.data.frame(sample_data(ps_adult_A))
adult_A_bray_dist_matrx = as.matrix(adult_A_bray_dist)
adult_A_bray_dist_df <- cbind(rownames(adult_A_bray_dist_matrx), data.frame(adult_A_bray_dist_matrx, row.names =NULL))
colnames(adult_A_bray_dist_df)[1] <- "SampleID"
colnames(adult_A_sample_data)[1]<-"SampleID"
adult_A_bray_joined <- inner_join(adult_A_bray_dist_df, adult_A_sample_data, by = "SampleID") 

adult_A_disp_Tray_Cage <- betadisper(adult_A_bray_dist, adult_A_bray_joined$Tray_Cage)
adult_A_perm_disp_Tray_Cage <- permutest(adult_A_disp_Tray_Cage, permutations = 999)
print(adult_A_perm_disp_Tray_Cage) # not signif


# B
ps_adult_B <- subset_samples(ps_adult, Building == "B-MK")
adult_B_bray_dist = phyloseq::distance(ps_adult_B, method="bray", weighted=F)
adult_B_sample_data <- as.data.frame(sample_data(ps_adult_B))
adult_B_bray_dist_matrx = as.matrix(adult_B_bray_dist)
adult_B_bray_dist_df <- cbind(rownames(adult_B_bray_dist_matrx), data.frame(adult_B_bray_dist_matrx, row.names =NULL))
colnames(adult_B_bray_dist_df)[1] <- "SampleID"
colnames(adult_B_sample_data)[1]<-"SampleID"
adult_B_bray_joined <- inner_join(adult_B_bray_dist_df, adult_B_sample_data, by = "SampleID") 

adult_B_disp_Tray_Cage <- betadisper(adult_B_bray_dist, adult_B_bray_joined$Tray_Cage)
adult_B_perm_disp_Tray_Cage <- permutest(adult_B_disp_Tray_Cage, permutations = 999)
print(adult_B_perm_disp_Tray_Cage) # not signif

#C

ps_adult_c <- subset_samples(ps_adult, Building == "C-ACC")
adult_c_bray_dist = phyloseq::distance(ps_adult_c, method="bray", weighted=F)
adult_c_sample_data <- as.data.frame(sample_data(ps_adult_c))
adult_c_bray_dist_matrx = as.matrix(adult_c_bray_dist)
adult_c_bray_dist_df <- cbind(rownames(adult_c_bray_dist_matrx), data.frame(adult_c_bray_dist_matrx, row.names =NULL))
colnames(adult_c_bray_dist_df)[1] <- "SampleID"
colnames(adult_c_sample_data)[1]<-"SampleID"
adult_c_bray_joined <- inner_join(adult_c_bray_dist_df, adult_c_sample_data, by = "SampleID") 

adult_c_disp_Tray_Cage <- betadisper(adult_c_bray_dist, adult_c_bray_joined$Tray_Cage)
adult_c_perm_disp_Tray_Cage <- permutest(adult_c_disp_Tray_Cage, permutations = 999)
print(adult_c_perm_disp_Tray_Cage) # not signif
...
```
# yep, reults are the same, or basically, the values differ slightly each time this is run (either way). The new code is 'better' but still really a similar number of lines.
